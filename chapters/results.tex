...
\par The results for the medium impedance spectra are given in figure \ref{fig:medium_model_IS_data}. The medium impedance spectra behaves as a resistor and capacitor elements in parallel with a single relaxation starting around 100 Mhz. In practice, an additional relaxation, governed by the polarization at the electrode-medium inteface (i.e. the EDL), would occur and largely mask the medium and cell relaxations. For these simulations the electric double layer phenomenon was excluded.
...
\par The simple FEA parametric study identified a max sensitivity of 0.0762 with an electrode width of 14 $\mu$m and an electrode gap of 4 $\mu$m. The study found that the electrode gap had larger effect than the electrode width and formed an optimal electrode gap "ridge" around 4-5 $\mu$m. 
...
\par In both the device and, to a lesser extent, the simple FEA results, there is apparent "mesh noise". COMSOL generates a new mesh for each simulation in the parametric study. What is referred to as "mesh noise" is the accumulation of effects on results caused by differences in meshes. Any mesh noise from the medium and mixture FEA studies are amplified through the calculations for sensitivity. The severity of "mesh noise" in the device FEA sensitivity is a clear signal that the meshing scheme needs to revisited, as the effect of the mesh is not within the tolerance required for the sensitivity curve. 
...
\par Sun's sensitivity and the simple FEA sensitivity are compared in figure \ref{fig:sunVsSimpleSensitivity}. Sun's method found a maximum sensitivity of 0.0684 at an electrode width and gap of 15 $\mu$m and 5 $\mu$m respectively, compared to the simple FEA maximum sensitivity of 0.0762 at an electrode width and gap of 14 $\mu$m and 4 $\mu$m respectively. The curves largely tracked each other except for electrode gap values less than 4 $\mu$m where the analytic solution shows the sensitivity steeply drop. This is where the maximum percent difference of 52.88\% is recorded. 
\par Since the power fraction does not share the same scale as Sun's and the simple FEA sensivity, all three studies were normalized by their maximum and compared in figure \ref{fig:sun-simple-powerFrac-comp}. The maximum power fraction was found at an electrode width and gap of 15 $\mu$m and 5 $\mu$m. The mean absolute error between the power fraction vs. Sun, the power fraction vs. the simple FEA, and Sun vs the simple FEA sensitivity curves are 0.0039, 0.0471, and 0.0486 respectively. This indicates that while the all three sensitivity studies track each other well, the Sun sensitivity and power fraction track each other very closely. This is supported by how all three studies predict nearly the same optimal electrode gap and width values inside the tested domain. 
...
\par The calculated impedance difference between the mixture and medium was also calculated and is presented in figure \ref{fig:expanded_impedance_difference}. By observing the impedance difference and the , impedance difference maximizes and plateaus at a low electrode gap. By increasing the electrode gap, power is distributed more evenly and progressively higher up in the center of the channel. Up to a certain point, this will increase the power in the region of a cell suspended above the channel. At the same time, increasing the electrode gap also continually increases the measured medium impedance even after the impedance of the cell is maximized. This leads to the characteristic "crest" of our sensitivity curve which designates the optimized electrode geometry.
...
\par Both the limiting values of the electrode gap and width are strongly effected by the channel height. With a small channel height, larger electrode gaps and widths are restricted from producing power in regions far above the cell, instead the power is focused into the small channel containing the cell. 
...
\par Figure \ref{fig:vertical_cell_position_sensitivity} tells a similar story. The sensitivity is maximized the closer that the cell is to the plane of the electrodes. However, at the expense of sensitivity, the electrode gap can be increased to decrease the effect cell height on the sensitivity. By increasing the electrode gap, the fringe effects of the electric field become smaller relative to the effective sensor zone and the power dissipation becomes increasingly homogeneous in region between the electrodes. 