...
\section{Maxwell's Mixture Theory}
 ...
 \end{figure}
  ...
  \par The permittivity of the single shelled cell in figure \ref{apx:apx:fig:single_shell_apx}, can be modelled as
...
\noindent Where $K(k)$ is the complete elliptic integral and is expressed as 
 ...
 \begin{flalign*}
 j\gamma &= \lim_{x \to 0} D_3 F(v(T_B + jx), k)&\\
 D_3 &= \lim_{x \to 0} \frac{F(v(T_B + jx),k)}{j\gamma}&
 \end{flalign*}
 ...
 \par Applying the imaginary-argument transform,
 ...
 \begin{flalign*}
    &\lim_{x \to 0} \frac{(T_B-T_D)(T_B + jx-T_A)}{(T_A-T_D)(T_B+jx-T_B)}  \\
    \\
    &= \lim_{x \to 0} \frac{(T_B-T_D)(T_B + jx-T_A)}{(T_A-T_D)jx} \;\;\;\;\;\;\; \text{where } T_D = 0  \\ 
    \\
    &= \lim_{x \to 0} \frac{T_B}{T_A}\frac{(T_B - T_A)}{jx} +\frac{T_B}{T_A} &
\end{flalign*}
...
\noindent Working from the inside out and starting with the definition of $\arcsin x$,
 \begin{flalign*}
    &\arcsin{x} = -j\ln(jx + \sqrt{1-x^2})&  \\
    \\
    &\arcsin(jx) = -j\ln(\sqrt{1+x^2}-x) \\ 
    \\
    &\lim_{x\to\infty} \arcsin(jx) = \lim_{x\to\infy} -j\ln(\sqrt{1+x^2}-x)\\
    \\
    &= \lim_{x\to\infty} -j\ln\bigg(\frac{(\sqrt{1+x^2}+x)(\sqrt{1+x^2}-x)}{(\sqrt{1+x^2}+x)}\bigg)\\
    \\
    &= \lim_{x\to\infty} -j\ln\bigg(\frac{1}{x(\sqrt{1/x^2+1}+1)}\bigg)&
\end{flalign*}
\begin{flalign*}
    &= -j\ln(0)&
\end{flalign*}
 ...
 \\
 \frac{dT}{dZ} &= 2\cosh\Big(\frac{Z\pi}{2h}\Big)\sinh\Big(\frac{Z\pi}{2h}\Big)\frac{\pi}{2h}& \text{Substitute equation \ref{apx:apx:eqn:TZ}}\\
 \\
