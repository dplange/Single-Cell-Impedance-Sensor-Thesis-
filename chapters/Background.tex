...
\end{equation}
 ...
 \begin{equation}
     Z = |Z| \angle \theta
 \end{equation}
 \noindent where $|Z|$ is the magnitude of the impedance vector and $\theta$ is the angle of the vector from the positive real axis. Given an AC current the voltage drop over the system can be expressed with Ohm's law as
 \begin{equation}
    \Delta \hat{V}(w) = Z(w)\hat{I}(w)
 \end{equation}
